...
backup() {
    ...
    local final_tar="${PWD}/backup-${hostname}-${timestamp}.tar.xz"

    echo "Creating backup of home directory..."
    echo "Temporary file: ${tmp_tar}"
    echo "Final location: ${final_tar}"
    echo "(Using xz compression for minimal size - this may take longer)"
    echo ""
    ...
    # Create tarball with xz compression (maximum compression)
    cd "$HOME" || return 1
    ...
    tar cf - $excludes $existing_includes -P | pv -s $total_bytes | xz -1 > "$tmp_tar"
