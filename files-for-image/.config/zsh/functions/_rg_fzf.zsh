_rg_fzf() {
  ...
  if [ -z "$pattern" ]; then
    ...
    | fzf --reverse --delimiter=$'\t' --with-nth=2.. --ansi)"

    local ret=$?

    # extract the original rg line before the formatting
  ...
  local raw_results
  raw_results="$(rg --line-number --column --no-heading $hidden_opt --color=never --smart-case --max-count=2 "$pattern" "$search_dir" 2>/dev/null)"
  ...
  # fancy formatting with columns
    ...
    | fzf --reverse --delimiter=$'\t' --with-nth=2.. --ansi --query="$pattern")"
