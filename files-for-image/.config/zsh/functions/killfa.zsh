killfa() {
  # killfa - fuzzy-find all processes and send signal
  # Usage:
  #   killfa <signal> [query]
  # Arguments:
  ...
  #   - Shows all processes (system-wide), not just user's processes

  setopt local_options pipefail

  _kill_fzf -a -c killfa "$@"
  local ret=$?
  if [ "$ret" -ne 0 ]; then
    return $ret
  fi
  ...
  if [ ${#pids[@]} -eq 0 ]; then
    printf '\e[31m%s\e[0m\n' "killfa: no processes selected" >&2
    return 1
  fi
